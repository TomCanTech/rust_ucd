...
impl Dictionary {
    fn from_sqlite(conn: &rusqlite::Connection) -> Result<Self> {
        let mut sys_map = make_hash(conn, "SELECT key, abbreviation, full FROM system_map")?;
        let mut pos_map = make_hash(conn, "SELECT key, abbreviation, full FROM pos_map")?;
        let mut dictionary = Dictionary {
            entries: vec![],
            system_map: Rc::new(RefCell::new(sys_map)),
            pos_map: Rc::new(RefCell::new(pos_map)),
        };
        let mut entry_retrieve_stmt = conn
...
mod tests {
    ...
    use crate::entry::entryfields::{EntryRelative, Headword};
    use crate::Entry;
    #[test]
    fn share_maps() {
        let mut dic_1 = Dictionary {
            ...
            system_map: Rc::new(RefCell::new(HashMap::from([
                ...
                (
                    2 as i32,
                    ("UCR".to_string(), "Unified Cornish Revised".to_string()),
                ),
            ]))),
        ...
        let entry_1 = Entry {
            ...
            relatives: Some(vec![
                EntryRelative::EntryAncestor(32),
                EntryRelative::EntryCousin(48),
            ]),
